...
for line in file_contents:
    if line.startswith("<"):
        if not (flag):
            if remove_counter != 0:
                for i in range(len(diff_values_alter), len(diff_values_plain)):
                    diff_values_alter.append(
                        [
                            diff_values_plain[i][0],
                            replace_number_with_zero(
                                diff_values_plain[i][1]).strip(),
                        ]
                    )
            flag = True
            remove_counter = 0
        ...
        if remove_counter == 0:
            diff_values_plain.append(
                [parts[1].strip(), replace_number_with_zero(parts[0][2:]).strip()]
            )
        else:
            remove_counter -= 1
...
for i in range(len(diff_values_alter)):
    ...
    if key in data:
        data[key] = [
            add_numbers_in_strings(data[key][0], diff_values_plain[i][1]),
            add_numbers_in_strings(data[key][1], diff_values_alter[i][1]),
        ]
    else:
...
comparison_table = tabulate(
    tabulate_array,
    headers=["Description", "Normal Compiler",
