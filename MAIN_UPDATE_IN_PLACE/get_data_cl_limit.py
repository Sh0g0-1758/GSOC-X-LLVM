...
value_str = os.environ.get('KNOB_VAL')

if value_str is not None:
    value_int = int(value_str)
    print(Fore.GREEN + "Value Set: ", value_int)
    print(Fore.RESET)
    directory_name = f"cl_limit_{value_int}"
else:
...
for i in range(1000):
    ...
    opt_command_vector = [
        './build/bin/opt', '-stats', f'test_{directory_name}.bc']
    opt_O1_command_vector = [
        './build/bin/opt', '-O1', '-stats', f'test_{directory_name}.bc']
    opt_O2_command_vector = [
        './build/bin/opt', '-O2', '-stats', f'test_{directory_name}.bc']
    opt_O3_command_vector = [
        './build/bin/opt', '-O3', '-stats', f'test_{directory_name}.bc']
    opt_Os_command_vector = [
        './build/bin/opt', '-Os', '-stats', f'test_{directory_name}.bc']
    opt_Oz_command_vector = [
        './build/bin/opt', '-Oz', '-stats', f'test_{directory_name}.bc']
